...
public class VisualizerView extends View {
    ...
    public void setVisualizerStyle(int style) {
        ...
        this.renderBins = (style == STYLE_WAVY) ? 32 : 48; // 48 bins for distinct spiky triangles
    ...
    public void updateVisualizer(byte[] bytes) {
        ...
        synchronized (this) {
            // Calculate frequency bands for both visualizer styles
            for (int i = 0; i < renderBins; i++) {
                ...
                if (visualizerStyle == STYLE_WAVY) {
                    amplitude = ratio * (width * 0.15f);
                } else {
                    amplitude = (float) Math.pow(ratio, 1.8f) * (width * 0.15f);
                }
